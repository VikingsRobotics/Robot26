...
struct wpi::Struct<VisionInfo> {
  ...
  static constexpr size_t kIdOffset =
      kCameraOffset + wpi::Struct<uint16_t>::GetSize();
  static constexpr size_t kConfidenceOffset =
      kIdOffset + wpi::Struct<uint16_t>::GetSize();
  static constexpr size_t kPoseOffset =
      kConfidenceOffset + wpi::Struct<float>::GetSize();
  static constexpr size_t kLatencyOffset =
      kPoseOffset + wpi::Struct<frc::Transform3d>::GetSize();
  
  static constexpr std::string_view GetTypeName() {
    return "struct:VisionInfo";
  }
  ...
  static void Pack(std::span<uint8_t> buffer, const VisionInfo& value) {
    wpi::PackStruct<kCameraOffset,uint16_t>(
        buffer,
        static_cast<uint16_t>(value.detectingCamera));
    ...
    wpi::PackStruct<kPoseOffset,frc::Transform3d>(
        buffer, static_cast<frc::Transform3d>(
        value.estimatedPose));
    wpi::PackStruct<kLatencyOffset,double>(
        buffer,
        value.estimatedLatency.value());
  }
